<template>
	...
	<div class="classify">
		<div class="g_att">
			<h1 class="t_title">
				<span class="size16">满减列表</span>
				<i>（ 共 {{dataTable.length}} 条记录 ）</i>
				<div @click="to" class="t_add fr">添加满减活动</div>
			</h1>
			<div class="notes-box">
...
export default {
	...
	methods: {
		...
		getData() {
			...
			this.$HTTP(this.$httpConfig.getfullCutList, this.params, 'post').then((res) => {
				this.dataTable = [];
		...
		deleteItem(value) {
			this.$confirm('此操作将永久删除此活动，是否继续？', '提示', {
				...
				this.$HTTP(this.$httpConfig.delFullCut, { id: value }, 'post').then((res) => {
					this.$message.success(res.data.message);
					this.getData();
				}).catch((err) => {
