		...
		<div class="g_att">
			...
			<el-table ref="multipleTable" :data="accountList" tooltip-effect="dark" style="width: 100%" @selection-change="handleSelectionChange">
				...
				<el-table-column label="最后登录时间" width="130" show-overflow-tooltip>
					<template slot-scope="scope">
						<time-Plunge :timePlunge="scope.row.last_login_time"></time-Plunge>
					</template>
				</el-table-column>
				...
				<el-table-column width="200" label="操作">
					<template slot-scope="scope" >
						<el-button v-show="scope.row.role!=null" @click="Jurisdiction(scope.row.group_id)" size="mini">权限</el-button>
						<el-button v-show="scope.row.role!=null" slot="reference" @click="edit(scope.row.id)" size="mini" icon="el-icon-edit"></el-button>
						<el-button v-show="scope.row.role!=null" size="mini" icon="el-icon-delete" @click="clear(scope.row.id)"></el-button>
...
export default {
	...
	methods: {
		...
		getData() {
			...
			this.$HTTP(this.$httpConfig.getAccountList,{page: 0}).then((res) => {
		...
		clear(id) {
			this.$confirm('此操作将永久删除该账号, 是否继续?', '提示', {
				...
				this.$HTTP(this.$httpConfig.delAccount,{id: id}).then((res) => {
					this.$message({
						duration: 1000,
						type: 'success',
