<template>
	<div class="logistics fl">
		...
		<div class="t_tab">
			<h1 class="t_title">
				<span v-if="!status" class="size16">添加最佳组合</span>
				<span v-else class="size16">修改最佳组合</span>
				<div class="t_m fr" @click="to">返回最佳组合</div>
			</h1>
...
export default {
	...
	methods: {
		...
		submit() {
			...
			if (typeof (this.hasCityId) == 'undefined' || this.hasCityId2.length < 1) {
				this.$message.error('请至少选择一个商品！！');
				return false;
			}
			if (this.status) {
				this.$HTTP(this.$httpConfig.saveCombo, {
					id: this.id,
				...
				this.$HTTP(this.$httpConfig.addCombo, {
					goods_id: this.hasCityId,
					sub_ids: this.hasCityId2,
				}, 'post').then((res) => {
					this.$message.success(res.data.message + ",即将跳转回列表页面");
