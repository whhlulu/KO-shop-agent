<template>
	...
	<div class="shopManage fl">
		<h1 class="titles">
			<span class="size16">推荐配件</span>
			<i>（ 共 {{dataTable.length}} 条记录 ）</i>
			<div class="t_add fr" @click="tolink">添加推荐配件</div>
		</h1>
		<div class="notes-box">
			<div class="notes_title">温馨提示</div>
			<div class="notes_content">
				<p>1.点击右上角添加按钮即可添加推荐商品</p>
				<p>2.主要商品只可选择1个，推荐商品可以选择多个</p>
			</div>
		</div>
		...
		<el-table border :data="dataTable" style="width: 100%" max-width="900">
			...
			<el-table-column prop="parts_num" label="配件数量">
			</el-table-column>
			...
			<el-table-column label="是否有效">
				<template slot-scope="scope">
					<el-switch @click.native="setStatus(scope.row.id,isStatus[scope.$index],scope.$index)" v-model="isStatus[scope.$index]" active-color="#13ce66" inactive-color="#ff4949">
					</el-switch>
				</template>
			</el-table-column>
...
export default {
	...
	methods: {
		setStatus(id, status, index) {
			this.$HTTP(this.$httpConfig.setPartsUse, {
				id: id,
		...
		getData() {
			this.$HTTP(this.$httpConfig.getPartsList, {
				id: this.goods_id,
		...
		deleteItem(value) {
			this.$confirm('此操作将永久删除此活动，是否继续？', '提示', {
				...
				this.$HTTP(this.$httpConfig.delParts, { id: value }, 'post').then((res) => {
					this.$message.success(res.data.message);
					this.getData();
				}).catch((err) => {
					this.$message.error(err);
